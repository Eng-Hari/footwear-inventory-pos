...
//     if (!billNumber || !invoiceRef.current) return toast.warn("⚠️ Save the sale first!");
//     try {
//       const { jsPDF } = await import("jsPDF");
//       invoiceRef.current.classList.remove("hidden");
//       const dataUrl = await htmlToImage.toPng(invoiceRef.current, { pixelRatio: 2, width: 226 });
//       invoiceRef.current.classList.add("hidden");

//       const pdf = new jsPDF({ orientation: "portrait", unit: "mm", format: [80, 297] });
//       const imgProps = pdf.getImageProperties(dataUrl);
//       pdf.addImage(dataUrl, "PNG", 0, 0, pdf.internal.pageSize.getWidth(), (imgProps.height * pdf.internal.pageSize.getWidth()) / imgProps.width);
...
//             <button onClick={handleDownloadPDF} disabled={!billNumber}
//               className="bg-purple-600 text-white px-6 py-3 rounded-lg w-full hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-lg font-semibold"
//             >Download PDF</button>
//           </div>
//         </div>
...
//           <Invoice cart={cart} subtotal={subtotal} total={total} discount={discount} paymentType={paymentType}
//             amountReceived={Number(amountReceived)} change={change} settings={settings} billNumber={billNumber} invoiceRef={invoiceRef}
//           />
//         </div>
//       </div>
//     </div>
